  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      for (var i = 1; i < count + 1; i++) {
        ...
        // <CardBody><div><img src={`https://fileup.infura-ipfs.io/ipfs/${landImg}`} alt="" width="90%" height="90%" style={{marginBottom:"10px"}}/><p>Area: {rowsArea[i-1]}</p><p>City: {rowsCity[i-1]}</p><p>State: {rowsState[i-1]}</p><p>PID: {rowsPID[i-1]}</p><p>Price: {rowsPrice[i-1]}</p>
        ...
        <div class="post-module">
          ...
          <div class="thumbnail">
            <div class="date">
            <div class="day">{i}</div>
                    </div><img src={`https://fileup.infura-ipfs.io/ipfs/${landImg}`}/>
          </div>
      ...
      <div class="post-meta"><span class="timestamp">Price: ₹ {rowsPrice[i-1]}</span></div>
                    <div class="post-meta"><span class="timestamp">View Verified Land  <a href={`https://fileup.infura-ipfs.io/ipfs/${document}`} target="_blank">Document</a></span></div>
          </div>
        </div>
      </Col>)
      }
